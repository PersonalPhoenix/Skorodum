...
def update_game(request, *args, **kwargs):
    selected_game = Game.objects.get(id=kwargs['pk'])

    # Обязательные поля.
    selected_game.name = kwargs['name']
    selected_game.theme = kwargs['theme']
    selected_game.client = kwargs['client']
    selected_game.date = kwargs['date']

    # Необязательные поля.
    selected_game.names_team_members = kwargs['names_team_members']
    selected_game.skip_emails = kwargs['skip_emails']
    selected_game.table_number = kwargs['table_number']
